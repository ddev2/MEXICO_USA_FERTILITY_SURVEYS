...
mujer1 <- mujer1 %>% dplyr::relocate(llave_muj, .before = 1)
mujer2 <- mujer2 %>% dplyr::relocate(llave_muj, .before = 1)
embarazos <- embarazos %>% dplyr::relocate(llave_muj, .before = 1)
mujer1 <- as.data.frame(lapply(mujer1, as.character))
mujer2 <- as.data.frame(lapply(mujer2, as.character))
embarazos <- as.data.frame(lapply(embarazos, as.character))
mujer1 <- convertToInteger(mujer1, 2)
mujer2 <- convertToInteger(mujer2, 2)
embarazos <- convertToInteger(embarazos, 2)
...
mujeres2014 <- dplyr::full_join(mujer1, mujer2, by = "llave_muj")
rm(mujer1)
rm(mujer2)

getDatos2014 <- function (mujeres) {
  datos <- data.frame(llave_muj=mujeres$llave_muj, region=mujeres$ent.x)
  datos$survey <- "ENADID2014"
  datos$surveyDate_cmc <- compute_cmc(10, 2014) # date of survey is from "del 4 de agosto al 26 de septiembre de 2014". We cut at October
  datos$monthBirth <- mujeres$p5_1_1
  datos$yearBirth <- mujeres$p5_1_2
  datos$age <- mujeres$p5_2
  datos$nLiveBirths <- mujeres$p5_9
  datos$nLiveBirths <- ifelse(is.na(datos$nLiveBirths),0,datos$nLiveBirths)
  datos$indiv_weight <- mujeres$fac_per.x
  ...
  datos$pregnant_wanted <- factor(datos$pregnant_wanted, levels = c(1, 2, 3), labels = c("yes", "yes but later", "no"))
  ...
  datos$mother_fecund <- factor(datos$mother_fecund, levels = c(1, 2, 3, 4, 5, 6, 9),
                                labels = c("health", "sterilized", "no partner", "menopause", "sterility", "other", "don't know"))
  datos$mother_unwanted <- mujeres$p7_16
  ...
  datos$mother_less <- mujeres$p7_17
  ...
  datos$ever_contraception <- factor(datos$ever_contraception, levels = c(1, 2),
                                     labels = c("yes", "no"))
  datos$age_first_sex <- mujeres$p8_36
  ...
  datos$lastUnion_month_start <- mujeres$p10_3_1
  datos$lastUnion_year_start <- mujeres$p10_3_2
  datos$lastUnion_month_end <- mujeres$p10_2_1
  datos$lastUnion_year_end <- mujeres$p10_2_2
  datos$lastUnion_cohab_before <- mujeres$p10_5
  datos$lastUnion_cohab_before <- factor(datos$lastUnion_cohab_before, levels = c(1,2,9),
                                         labels = c("yes", "no", "don't know"))
  datos$lastUnion_month_cohab_before <- mujeres$p10_6_1
  datos$lastUnion_year_cohab_before <- mujeres$p10_6_2
  datos$nUnionBeforeLast <- mujeres$p10_8
  datos$firstUnionNotLast_month_start <- mujeres$p10_9_1
  datos$firstUnionNotLast_year_start <- mujeres$p10_9_2
  datos$firstUnionNotLast_month_end <- mujeres$p10_11_1
  datos$firstUnionNotLast_year_end <- mujeres$p10_11_2
  datos$firstUnionNotLast_end_motive <- mujeres$p10_10
...
bigDataChildbirths <- function (embarazos) {
  ...
  missing_keys <- setdiff(ENADID_P_wide$llave_muj, ENADID2014$llave_muj)
...
ENADID2014_full <- dplyr::left_join(ENADID2014, ENADID_P, by = "llave_muj")
ENADID2014_full$nChildren[is.na(ENADID2014_full$nChildren)] <- 0
if (!all.equal(ENADID2014_full$nChildren, ENADID2014_full$nBioKids)) stop ("nBioKids and nChildren do not match")
rm(ENADID_P)
rm(ENADID2014)
...
save(ENADID2014_full, file=path_ENADID2014)
save(mujeres2014, file=path_ENADID2014_mujeres)
