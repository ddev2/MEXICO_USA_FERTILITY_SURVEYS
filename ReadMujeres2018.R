...
mujeres2018 <- dplyr::full_join(mujer1, mujer2, by = "llave_muj")
rm(mujer1)
rm(mujer2)

getDatos2018 <- function (mujeres) {
  datos <- data.frame(llave_muj=mujeres$llave_muj, region=mujeres$ent.x)
  datos$survey <- "ENADID2018"
  datos$surveyDate_cmc <- compute_cmc(8, 2018) # date of survey is from "2 de abril al 13 de julio de 2018". We cut at August 2018
  ...
  datos$ever_contraception <- factor(datos$ever_contraception, levels = c(1, 2),
                                     labels = c("yes", "no"))
  datos$age_first_sex <- mujeres$p8_38
...
bigDataChildbirths <- function (embarazos) {
  ...
  missing_keys <- setdiff(ENADID_P_wide$llave_muj, ENADID2018$llave_muj)
...
ENADID2018_full <- dplyr::left_join(ENADID2018, ENADID_P, by = "llave_muj")
ENADID2018_full$nChildren[is.na(ENADID2018_full$nChildren)] <- 0
if (!all.equal(ENADID2018_full$nChildren, ENADID2018_full$nBioKids)) stop ("nBioKids and nChildren do not match")
rm(ENADID2018)
rm(ENADID_P)
...
save(ENADID2018_full, file=path_ENADID2018)
save(mujeres2018, file=path_ENADID2018_mujeres)
